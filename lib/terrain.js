...
function makeFill(size, func) {
  lazyProperty(module, 'exports', function(materials) {
...
//Terrain
makeFill([33,33,33], function(i,j,k) {
  if (i===30 && j===30 && k===30) {
    return OPAQUE|6; // wool
  }
  ...
  if(h0 <= j) {
    return OPAQUE|5//0x19 // grass
  }
  var h1 = 2.0 * Math.sin(Math.PI * i * 0.25 - Math.PI * k * 0.3) + 20
  if(h1 <= j) {
    return OPAQUE|0//0x20 // dirt
  }

  // stone with ores
  if(4 < j) {
    return Math.random() < 0.1 ?
        OPAQUE|4 : //0x23 : // diamond
        OPAQUE|1 //0x10 // stone
  }
  return OPAQUE|3 // lava
})
