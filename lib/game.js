...
Game.prototype.updatePlayerPhysics = function(controls) {
  ...
  if (cs.left[0]) freedom['x+'] = false
  if (cs.right[0]) freedom['x-'] = false
  if (cs.up[0]) freedom['y+'] = false
  if (cs.down[0]) freedom['y-'] = false
  if (cs.forward[0]) freedom['z-'] = false
  if (cs.back[0]) freedom['z+'] = false
  
  var ry = this.controls.yawObject.rotation.y
  
  if (cs.left[0] && !cs.right[0]) {
    controls.yawObject.position.x += Math.cos(ry - Math.PI / 2)
  }
  if (cs.right[0] && !cs.left[0]) {
    controls.yawObject.position.x += Math.cos(ry + Math.PI / 2)
  }
  
  if (cs.forward[0] && !cs.back[0]) {
    controls.yawObject.position.z += Math.sin(ry + Math.PI)
  }
  if (cs.back[0] && !cs.forward[0]) {
    controls.yawObject.position.z += Math.sin(ry - Math.PI)
  }
}
