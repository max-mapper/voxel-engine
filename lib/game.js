var Game = (function() {
  ...
  function Game(voxelSource) {
    ...
    this.material = this.loadTextures([ 'grass', 'grass_dirt' ])
  ...
  Game.prototype.raycast = function() {
    ...
    var direction = this.camera.matrixWorld.multiplyVector3(new THREE.Vector3(0,0,-100))
    var raycaster = new THREE.Raycaster(start, direction)
    var intersects = raycaster.intersectObject(this.currentWireMesh())
    
    var material = new THREE.LineBasicMaterial({ color: 0xff0000 })
    var geometry = new THREE.Geometry()
    geometry.vertices = [start, direction]
  ...
  Game.prototype.currentWireMesh = function() {
    var cid = game.chunker.chunkAtPosition(game.controls.yawObject.position).join('|')
    return game.chunker.meshes[cid].wireMesh
  }
