var Game = (function() {
  ...
  function Game(voxelSource) {
    ...
    this.startingPosition = new THREE.Vector3(0,850,0)
  ...
  Game.prototype.tick = function() {
    ...
    this.controls.update(dt, function (pos, velocity) {
      ...
      var x = Math.abs(~~(pos.x / self.cubeSize))
      var y = Math.abs(~~(pos.y / self.cubeSize))
      var z = Math.abs(~~(pos.z / self.cubeSize))

      var v = chunk.voxels[x + y*32 + z*32*32]
      if (v) {
        console.log(x,y,z,"IN VOXEL")
        this.gravityEnabled = false
        velocity.y = 0
      } else {
        console.log(x,y,z,"NO VOXEL")
