...
Game.prototype.updatePlayerPhysics = function(controls) {
  ...
  var blocks = [], blockIndexes = {};
  collisions.top.concat(collisions.bottom).forEach(function (c) {
    var v = self.voxels.voxelIndex(c)
    if (blockIndexes[v]) return
    blockIndexes[v] = true
    
    blocks.push(new THREE.Vector3(
      Math.floor(c.x / self.cubeSize) * self.cubeSize,
      Math.floor(c.y / self.cubeSize) * self.cubeSize,
      Math.floor(c.z / self.cubeSize) * self.cubeSize
    ))
  })
  
  var prevdom = controls.freedom
  var freedom = controls.freedom = {
    'x+': true, 'y+': true, 'z+': true,
    'x-': true, 'y-': true, 'z-': true
  }
  
  var bpos = new THREE.Vector3(
    Math.floor(pos.x / self.cubeSize) * self.cubeSize,
    Math.floor(pos.y / self.cubeSize) * self.cubeSize,
    Math.floor(pos.z / self.cubeSize) * self.cubeSize
  )
  blocks.forEach(function (block) {
    var xeq = block.x === bpos.x
    var yeq = block.y === bpos.y
    var zeq = block.z === bpos.z
    
    if (yeq && zeq) {
      if (block.x > bpos.x) freedom['x+'] = false
      if (block.x < bpos.x) freedom['x-'] = false
    }
    if (xeq && zeq) {
      if (block.y > bpos.y) freedom['y+'] = false
      if (block.y < bpos.y) freedom['y-'] = false
    }
    if (xeq && yeq) {
      if (block.z > bpos.z) freedom['z+'] = false
      if (block.z < bpos.z) freedom['z-'] = false
    }
  })
}
