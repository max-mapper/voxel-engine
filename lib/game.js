...
Game.prototype.updatePlayerPhysics = function(controls) {
  ...
  var actual = controls.yawObject
  var player = actual.clone()
  player.translateY(-30)
  var velocity = controls.velocity
  
  player.translateX(velocity.x)
  if (!this.getBlock(player.position)) actual.translateX(velocity.x)
  else player.translateX(-velocity.x)

  player.translateY(velocity.y)
  if (!this.getBlock(player.position)) actual.translateY(velocity.y)
  else player.translateY(-velocity.y)

  player.translateZ(velocity.z) 
  if (!this.getBlock(player.position)) actual.translateZ(velocity.z)
  else player.translateZ(-velocity.z) 
