...
Game.prototype.updatePlayerPhysics = function(controls) {
  ...
  collisions.top.forEach(function (c) {
    var v = self.voxels.voxelIndex(c)
    if (blockIndexes.indexOf(v) < 0) {
      blockIndexes.push(v)
      blocks.push(c)
    }
  })
  collisions.bottom.map(function (c) {
    var v = self.voxels.voxelIndex(c)
    if (blockIndexes.indexOf(v) < 0) {
      blockIndexes.push(v)
      blocks.push(c)
  ...
  var prevFreedom = controls.freedom
  controls.freedom = {
  ...
  blocks.forEach(function (block) {
    if (block.x > pos.x) controls.freedom['x+'] = false
    if (block.y > pos.y) controls.freedom['y+'] = false
    if (block.z > pos.z) controls.freedom['z+'] = false
    if (block.x < pos.x) controls.freedom['x-'] = false
    if (block.y < pos.y) controls.freedom['y-'] = false
    if (block.z < pos.z) controls.freedom['z-'] = false
  })
